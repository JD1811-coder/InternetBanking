    ...
    if (isset($_POST['open_account'])) {
        ...
        while ($stmt_check->fetch()) {
            if (trim($existing_acc_type) === "Joint Account") { // Ensure matching format
                $joint_acc_count++; // Count joint accounts
            }
        }
        $stmt_check->close();
    
        // Validation logic: Only allow Joint Accounts (max 3)
        if (strcasecmp($acc_type, "Joint Account") !== 0) { // Case-insensitive check
            $err = "You are only allowed to open Joint Accounts.";
        } elseif ($joint_acc_count >= 3) {
            $err = "You can only have a maximum of 3 Joint Accounts.";
        } else {
            // Allow Joint Account creation
            ...
            if ($stmt) {
                $success = "Joint Account Opened Successfully";
            } else {
